...
namespace RenderHeads.Media.AVProVideo.Editor
{
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (propIsOverrideTransition.boolValue)
			{
				...
				if (propTransitionMode.enumValueIndex != (int)PlaylistMediaPlayer.Transition.None)
				{
					...
					EditorGUILayout.PropertyField(property.FindPropertyRelative("overrideTransitionEasing.preset"), _guiTextEasing);
				}
				EditorGUI.indentLevel--;
			}
		}
	...
	public class PlaylistMediaPlayerEditor : UnityEditor.Editor
	{
		...
		private void OnEnable()
		{
			...
			_propNextTransition = this.CheckFindProperty("_nextTransition");
			_propTransitionDuration = this.CheckFindProperty("_transitionDuration");
			_propTransitionEasing = this.CheckFindProperty("_transitionEasing.preset");
