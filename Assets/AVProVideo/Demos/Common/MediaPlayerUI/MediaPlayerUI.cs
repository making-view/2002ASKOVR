...
//-----------------------------------------------------------------------------
// Copyright 2018-2020 RenderHeads Ltd.  All rights reserverd.
//-----------------------------------------------------------------------------
			...
			if (_mediaPlayer.Info != null)
			{
				...
				if (_segmentsBuffered)
				{
					...
					if (times.Count > 0 && timelineRange.duration > 0.0)
					{
						...
						{
							ranges[i * 2 + 0] = (float)((times[i].StartTime - timelineRange.startTime) / timelineRange.duration);
							ranges[i * 2 + 1] = (float)((times[i].EndTime - timelineRange.startTime) / timelineRange.duration);
						}
					}
