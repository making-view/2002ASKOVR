#if UNITY_EDITOR_OSX || (!UNITY_EDITOR && (UNITY_STANDALONE_OSX || UNITY_IOS || UNITY_TVOS))
...
namespace RenderHeads.Media.AVProVideo
{
	public sealed partial class AppleMediaPlayer: BaseMediaPlayer
	{
		...
		public AppleMediaPlayer(MediaPlayer.OptionsApple options)
		{
			...
			switch (options.preferredMaximumResolution)
			{
				case MediaPlayer.OptionsApple.PreferredMaximumResolution.NoPreference:
					break;
				case MediaPlayer.OptionsApple.PreferredMaximumResolution._480p:
					_playerSettings.preferredMaximumResolution_width = 640;
					_playerSettings.preferredMaximumResolution_height = 480;
					break;
				case MediaPlayer.OptionsApple.PreferredMaximumResolution._720p:
					_playerSettings.preferredMaximumResolution_width = 1280;
					_playerSettings.preferredMaximumResolution_height = 720;
					break;
				case MediaPlayer.OptionsApple.PreferredMaximumResolution._1080p:
					_playerSettings.preferredMaximumResolution_width = 1920;
					_playerSettings.preferredMaximumResolution_height = 1080;
					break;
				case MediaPlayer.OptionsApple.PreferredMaximumResolution._2160p:
					_playerSettings.preferredMaximumResolution_width = 3840;
					_playerSettings.preferredMaximumResolution_height = 2160;
					break;
				case MediaPlayer.OptionsApple.PreferredMaximumResolution.Custom:
					#if UNITY_2017_2_OR_NEWER
					_playerSettings.preferredMaximumResolution_width = options.customPreferredMaximumResolution.x;
					_playerSettings.preferredMaximumResolution_height = options.customPreferredMaximumResolution.y;
					#endif
					break;
			}
			...
			_flags = _flags.SetAutoPauseOff(options.flags.ResumePlaybackAfterAudioSessionRouteChange());
	...
	public sealed partial class AppleMediaPlayer
	{
		...
		internal partial struct Native
		{
			...
			internal enum AVPPlayerFlags : int
			{
				...
				AutoPauseOff          = 1 << 16,	// iOS only, resumes playback after audio session route change
				Dirty                 = 1 << 31
			}
	...
	internal static class AppleMediaPlayerExtensions
	{
		...
		internal static AppleMediaPlayer.Native.AVPPlayerStatus SetHasUpdatedText(this AppleMediaPlayer.Native.AVPPlayerStatus status, bool b)
		{
			if (status.HasUpdatedText() ^ b)
			{
				status = b ? status |  AppleMediaPlayer.Native.AVPPlayerStatus.UpdatedText
				           : status & ~AppleMediaPlayer.Native.AVPPlayerStatus.UpdatedText;
			}
			return status;
		}
		...
		internal static bool IsAutoPauseOff(this AppleMediaPlayer.Native.AVPPlayerFlags flags)
		{
			return (flags & AppleMediaPlayer.Native.AVPPlayerFlags.AutoPauseOff) == AppleMediaPlayer.Native.AVPPlayerFlags.AutoPauseOff;
		}

		internal static AppleMediaPlayer.Native.AVPPlayerFlags SetAutoPauseOff(this AppleMediaPlayer.Native.AVPPlayerFlags flags, bool b)
		{
			if (flags.IsAutoPauseOff() ^ b)
			{
				flags = (b ? flags | AppleMediaPlayer.Native.AVPPlayerFlags.AutoPauseOff
				           : flags & ~AppleMediaPlayer.Native.AVPPlayerFlags.AutoPauseOff) | AppleMediaPlayer.Native.AVPPlayerFlags.Dirty;
			}
			return flags;
		}
	...
	public sealed partial class AppleMediaPlayer
	{
		...
		{
			SeekWithTolerance((float)toTime, double.PositiveInfinity, double.PositiveInfinity);
		}
		...
		{
			Native.AVPPlayerSetDecryptionKey(_player, key, key.Length);
		}
	...
	public sealed partial class AppleMediaPlayer
	{
		...
		private static Matrix4x4 YCbCrMatrix_ITU_R_709 = new Matrix4x4(
			new Vector4( 1.0000f,  1.0000f,  1.0000f,  0.0000000000f),
			new Vector4( 0.0000f, -0.3441f,  1.7720f, -0.5019608140f),
			new Vector4( 1.4020f, -0.7141f,  0.0000f, -0.5019608140f),
			Vector4.zero
		);
	...
	public sealed partial class AppleMediaPlayer
	{
		internal class Logger
		{
...
#endif
			private delegate void DebugLogCallbackDelegate(Native.AVPLogLevel level, IntPtr ptr);

#if UNITY_IOS || UNITY_TVOS || ENABLE_IL2CPP
			[MonoPInvokeCallback(typeof(DebugLogCallbackDelegate))]
#endif
			private static void DebugLogCallback(Native.AVPLogLevel level, IntPtr ptr)
			{
				string str = Marshal.PtrToStringUni(ptr);
				if (str != null)
				{
					if (level == Native.AVPLogLevel.Error)
					{
						Debug.LogErrorFormat("[AVProVideo] {0}", str);
					}
					else if (level == Native.AVPLogLevel.Warning)
					{
						Debug.LogWarningFormat("[AVProVideo] {0}", str);
					}
					else
					{
						Debug.LogFormat("[AVProVideo] {0}", str);
					}
				}
			}
			...
			internal Logger()
			{
				...
				Native.AVPPluginSetLogFunction(func, Native.StringEncoding.UTF16);
			}

			~Logger()
			{
				Native.AVPPluginSetLogFunction(IntPtr.Zero, Native.StringEncoding.UTF16);
			}
		}

		internal static Logger _logger;

		internal static void InitialiseLogging()
		{
			_logger = new Logger();
		}
		...
		internal partial struct Native
		{
			internal enum StringEncoding: int
			{
				ASCII = 1,
				UTF8  = 4,
				UTF16 = 10
			...
			internal enum AVPLogLevel: int
			{
				Off     =  0,
				Error   = AVPLogFlag.Error,
				Warning = AVPLogFlag.Warning | Error,
				Info    = AVPLogFlag.Info    | Warning,
				Debug   = AVPLogFlag.Debug   | Info,
				Verbose = AVPLogFlag.Verbose | Debug,
			}

			[DllImport(PluginName)]
			internal static extern void AVPPluginSetLogFunction(IntPtr function, StringEncoding encoding);

			[DllImport(PluginName)]
			internal static extern void AVPPluginSetLogLevel(AVPLogLevel level);
		}
	}
