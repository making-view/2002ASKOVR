...
#if UNITY_EDITOR || UNITY_STANDALONE_OSX || UNITY_STANDALONE_WIN || UNITY_IOS || UNITY_TVOS || UNITY_ANDROID
...
namespace RenderHeads.Media.AVProVideo
{
	...
	/// Everything related to setting up materials for rendering videos
	public class VideoRender
	{
		...
		public static readonly LazyShaderProperty PropCroppingScalars = new LazyShaderProperty("_CroppingScalars");
		...
		public static void SetupCroppingScalar(Material material, float[] transform)
		{
#if (!UNITY_EDITOR && UNITY_ANDROID)
			if (material != null && (material.HasProperty(VideoRender.PropCroppingScalars.Id)))
			{
				if (transform != null)
				{
					material.SetVector(VideoRender.PropCroppingScalars.Id, new Vector4(transform[0], transform[3], 1.0f, 1.0f));
				}
				else
				{
					material.SetVector(VideoRender.PropCroppingScalars.Id, new Vector4(1.0f, 1.0f, 1.0f, 1.0f));
				}
			}
#endif
		}
		...
		public static void SetupMaterialForMedia(Material material, MediaPlayer mediaPlayer, int texturePropId = -1)
		{
			...
			{
				material.SetTexture(texturePropId, null);
			...
			VideoMapping mapping = VideoMapping.Normal, StereoPacking stereoPacking = StereoPacking.None, AlphaPacking alphaPacking = AlphaPacking.None)
		{
			...
			#if (!UNITY_EDITOR && UNITY_ANDROID)
			if (material.HasProperty(VideoRender.PropCroppingScalars.Id))
			{
				VideoRender.SetupCroppingScalar(material, textureTransform);
			}
			#endif
