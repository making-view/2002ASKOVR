...
#region PlatformOptions
		...
		public class OptionsApple: PlatformOptions
		{
			...
			public enum PreferredMaximumResolution
			{
				NoPreference,
				_480p,
				_720p,
			...
			public PreferredMaximumResolution preferredMaximumResolution;
			...
			{
				if (preferredPeakBitRate > 0.0f)
				{
					switch (preferredPeakBitRateUnits)
					{
						case BitRateUnits.bps:
							return (double)preferredPeakBitRate;
						case BitRateUnits.Kbps:
							return (double)preferredPeakBitRate * 1000.0;
						case BitRateUnits.Mbps:
							return (double)preferredPeakBitRate * 1000000.0;
						case BitRateUnits.Gbps:
							return (double)preferredPeakBitRate * 1000000000.0;
					}
				}
			...
			public OptionsApple(TextureFormat defaultTextureFormat, Flags defaultFlags)
			{
				...
				flags = defaultFlags;
				preferredMaximumResolution = PreferredMaximumResolution.NoPreference;
				#if UNITY_2017_2_OR_NEWER
				customPreferredMaximumResolution = Vector2Int.zero;
				#endif
				preferredPeakBitRate = 0.0f;
				preferredPeakBitRateUnits = BitRateUnits.Kbps;
			}
